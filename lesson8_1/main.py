...
class StockMonitorApp:
    ...
    def create_stock_card(self, stock_code: str):
        ...
        card_frame.configure(style='White.TLabelframe')  # 設定自訂樣式
...
更新時間: {stock_data.get('update_time', 'N/A')}
            """
            ttk.Label(card_frame, text=info_text, justify=tk.LEFT, background='white', font=('Arial', 13)).pack(side=tk.LEFT)
        else:
            ttk.Label(card_frame, text="等待更新資料...", background='white', font=('Arial', 13)).pack(side=tk.LEFT)
